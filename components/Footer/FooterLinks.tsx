...
const FooterLinks = ({ locations, categories, resourceLinks }: FooterLinksProps) => {
  return (
    <div className="flex items-start gap-10 md:gap-8 w-full md:justify-between flex-wrap md:flex-nowrap">
      {/* Locations */}
      <FooterLinkSection 
        title="Locations" 
        links={locations.map(text => ({ text, href: "#" }))} 
      ...
      <FooterLinkSection 
        title="Niches" 
        links={categories.map(text => ({ text, href: "#" }))} 
...
const FooterLinkSection = ({ title, links, animationDelay }: FooterLinkSectionProps) => {
  return (
    <div className={cn(
